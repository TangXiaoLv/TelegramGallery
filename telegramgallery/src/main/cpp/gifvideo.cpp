...
jint Java_com_tangxiaolv_telegramgallery_AnimatedFileDrawable_createDecoder(JNIEnv *env,
                                                                            jclass clazz,
                                                                            jstring src,
                                                                            jintArray data) {
...
void Java_com_tangxiaolv_telegramgallery_AnimatedFileDrawable_destroyDecoder(JNIEnv *env,
                                                                             jclass clazz,
                                                                             jobject ptr) {
    if (ptr == NULL) {
        return;
...
jint Java_com_tangxiaolv_telegramgallery_AnimatedFileDrawable_getVideoFrame(JNIEnv *env,
                                                                            jclass clazz,
                                                                            jobject ptr,
                                                                            jobject bitmap,
                                                                            jintArray data) {
    ...
    while (true) {
        ...
        if (info->pkt.size > 0) {
            ...
            if (got_frame == 0) {
                if (info->has_decoded_frames) {
                    ...
                    if ((ret = avformat_seek_file(info->fmt_ctx, -1,
        ...
        if (got_frame) {
            ...
            if (info->frame->format == AV_PIX_FMT_YUV420P ||
                ...
                jint *dataArr = env->GetIntArrayElements(data, 0);
                if (dataArr != nullptr) {
                    dataArr[2] = (int) (1000 * info->frame->pkt_pts *
